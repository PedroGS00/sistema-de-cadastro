...
class PostgresIntegrationTest {
    ...
    void setupDatabase() {
        try (Connection conn = dataSource.getConnection()) {
            ...
            try (Statement st = conn.createStatement()) {
                ...
                // Criação de tabela products compatível com a entidade
                st.execute("""
    ...
    void cleanupDatabase() {
        try (Connection conn = dataSource.getConnection(); Statement st = conn.createStatement()) {
            ...
            // Não falha os testes por erro de limpeza, mas loga
    ...
    void deveValidarSchemaDoBanco() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            ...
            // Verifica existência de constraints UNIQUE básicas via information_schema
    ...
    void deveExecutarCrudCompletoDeUsuario() {
        ...
        assertNotNull(saved.getId(), "ID deve ser gerado ao salvar usuário");

        // READ
        Optional<User> byId = userRepository.findById(saved.getId());
        assertTrue(byId.isPresent(), "Usuário deve ser recuperado por ID");
        assertEquals("fulano@example.com", byId.get().getEmail(), "Email deve ser mantido");

        // UPDATE
        saved.setCity("São Paulo");
        saved.setAddress("Rua A, 100");
        User updated = userRepository.save(saved);
        assertEquals("São Paulo", updated.getCity(), "Cidade deve ser atualizada");

        // DELETE
        userRepository.deleteById(updated.getId());
        assertFalse(userRepository.findById(updated.getId()).isPresent(), "Usuário deve ser removido");
    }
    ...
    void deveExecutarCrudCompletoDeProduto() {
        ...
        // UPDATE (operações de integração via regra de negócio)
    ...
    void deveFalharAoInserirUsuarioComEmailDuplicado() {
        ...
        Exception ex = assertThrows(Exception.class, () -> userRepository.saveAndFlush(u2),
                "Deve lançar exceção devido à UNIQUE constraint do email");
        assertNotNull(ex, "Exceção não deve ser nula");
    }
