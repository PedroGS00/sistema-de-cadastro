...
class ExternalHttpVcrTest {
    ...
    void testVCRRecordAndPlayback() throws Exception {
        ...
        // Modo AUTO: grava se não existir, reproduz se existir
    ...
    void testGet404() throws Exception {
        ...
        // Observação: EasyVCR com HttpURLConnection pode lançar NPE ao tentar instrumentar respostas 4xx.
        // Validamos aqui que o erro é propagado e é consistente em Replay (idempotência de falha).
        ...
        try {
            ...
            // Workaround para limitação do EasyVCR com respostas não-2xx
            assertNotNull(npe.getMessage());
        }
        boolean exists404 = VcrService.cassetteExists(cassetteName);

        // Reproduz: esperamos o mesmo tipo de falha
    ...
    void testGet500() throws Exception {
        ...
        // Observação: EasyVCR com HttpURLConnection pode lançar NPE ao tentar instrumentar respostas 5xx.
    ...
    void testPostJsonWithHeaders() throws Exception {
        ...
        assertEquals(200, code);
        String body = readBody(conn);
        // Verificação mais tolerante à variação de resposta do httpbin
        assertNotNull(body);
        assertTrue(body.length() > 0);
        assertTrue(VcrService.cassetteExists(cassetteName));

        // Reproduz e valida idempotência
