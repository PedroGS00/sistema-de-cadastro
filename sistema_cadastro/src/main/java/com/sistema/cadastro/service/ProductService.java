...
public class ProductService {
    ...
    public Page<Product> getAllProducts(Pageable pageable) {
        log.info("Listando produtos com paginação: página {}, tamanho {}", 
    ...
    public Page<Product> getLowStockProducts(Integer threshold, Pageable pageable) {
        log.info("Buscando produtos com baixo estoque paginado (limite: {}), página {} tamanho {}",
    ...
    private void validateProductCreation(ProductDTO productDTO) {
        ...
        if (productRepository.existsBySku(cleanSku)) {
            throw new BusinessException("SKU já cadastrado", org.springframework.http.HttpStatus.CONFLICT, "DUPLICATE_SKU");
        }
    ...
    private void validateProductUpdate(Long id, ProductDTO productDTO) {
        // Valida SKU único (exceto para o próprio produto)
        String cleanSku = productDTO.getSku().toUpperCase().trim();
        if (productRepository.findBySkuAndIdNot(cleanSku, id).isPresent()) {
            throw new BusinessException("SKU já cadastrado para outro produto", org.springframework.http.HttpStatus.CONFLICT, "DUPLICATE_SKU");
        }
    ...
    public Product updateStock(Long id, Integer quantity) {
        log.info("Requisição de atualização de estoque: id={}, quantity={}", id, quantity);
